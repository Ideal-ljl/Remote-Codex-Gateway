...
- 只有显式调大 `CODEXMANAGER_ROUTE_HEALTH_P2C_ORDERED_WINDOW` 时，头部候选才可能被“更健康”的前几个候选之一替换到第一位

适用理解：

- 更接近“按账号优先级严格尝试”
- 新会话没有 conversation binding 且没有手动指定账号时，会先尝试候选池第一位
...
- 把协议兼容回归固定到 Rust 测试路径
- 持续保持 `/v1/models` 与平台模型目录之间的行为对齐
