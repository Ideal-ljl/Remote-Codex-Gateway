...
fn model_route_error(
    ...
    bootstrap_model_routes_for_plan(storage, execution_plan, false);
    let mut model_exists = storage
        .model_catalog_model_exists("default", model)
        .map_err(|err| (500, format!("model_catalog_read_failed: {err}")))?;
    if !model_exists && plan_can_refresh_account_catalog(execution_plan) {
        bootstrap_model_routes_for_plan(storage, execution_plan, true);
        model_exists = storage
            .model_catalog_model_exists("default", model)
            .map_err(|err| (500, format!("model_catalog_read_failed: {err}")))?;
    }
    if !model_exists {
    ...
    let mut has_upstream_source_match =
        direct_upstream_model_matches_route(storage, model, &route_source_kinds)
            .map_err(|err| (500, format!("source_model_read_failed: {err}")))?;
    if !has_upstream_source_match && plan_can_refresh_account_catalog(execution_plan) {
        bootstrap_model_routes_for_plan(storage, execution_plan, true);
        has_upstream_source_match =
            direct_upstream_model_matches_route(storage, model, &route_source_kinds)
                .map_err(|err| (500, format!("source_model_read_failed: {err}")))?;
    }
...
fn bootstrap_model_routes_for_plan(
    ...
    match execution_plan.route_kind {
        GatewayUpstreamRouteKind::AccountRotation => {
            let _ = crate::apikey_models::bootstrap_account_pool_model_routes(
                storage,
                allow_remote_account_catalog_fetch,
            );
        }
        GatewayUpstreamRouteKind::AggregateApi => {
...
fn plan_can_refresh_account_catalog(
    execution_plan: super::executor::GatewayUpstreamExecutionPlan,
) -> bool {
    matches!(
        execution_plan.route_kind,
