...
fn is_same_origin_host(origin: &str, host: Option<&str>) -> bool {
    ...
    let Ok(host_url) = Url::parse(&host_url) else {
        return false;
    };
...
fn is_allowed_origin(headers: &HeaderMap, origin: &str) -> bool {
...
fn validate_axum_headers(headers: &HeaderMap) -> Option<AxumResponse> {
    ...
    if let Some(origin) = headers
        ...
        .map(str::trim)
    {
        if !is_allowed_origin(headers, origin) {
            return Some((StatusCode::FORBIDDEN, "{}").into_response());
        }
    }
